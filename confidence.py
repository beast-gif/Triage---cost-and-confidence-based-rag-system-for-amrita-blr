...
from confidence_V2 import compute_confidence_v2
...
async def score_query(query: str, top_k: int = TOP_K) -> dict:
    ...
    RETRIEVAL AND THE ENSEMBLE RUN CONCURRENTLY
    -------------------------------------------
    They are independent: retrieve_for() decides its route from the query text
    alone (wants_department_head / department_in_query), never from the
    ensemble's category label. So there is no reason to wait for one before
    starting the other.

    Sequential, the wall clock was retrieval + ensemble. Concurrent, it is
    max(retrieval, ensemble) — and since the ensemble makes three network calls
    while retrieval is local compute, the overlap is close to free. Torch
    releases the GIL during inference, so the worker thread genuinely runs
    alongside the event loop rather than blocking it.

    An ensemble failure is caught rather than propagated: agreement drops to
    0.0, which discounts confidence instead of taking the whole query down.
    """
    retrieval_task = asyncio.to_thread(_retrieve_and_rerank, query, top_k)
    ensemble_task = get_ensemble_votes(query)

    retrieval_result, votes = await asyncio.gather(
        retrieval_task, ensemble_task, return_exceptions=True
    )
    ...
    candidates, reranked, all_scores, route, filtered = retrieval_result

    if not candidates:
        return {
            "final_confidence": 0.0, "band": "low", "route": route,
            "routing_label": "general_info", "generator_mode": "disambiguate",
            "retrieval_details": {}, "ensemble_details": {}, "chunks": [],
            "candidates_retrieved": 0,
            "top_result": {"source_url": None, "heading": None,
                           "content_preview": None},
            "note": "no candidates retrieved",
        }

    ensemble = ensemble_agreement(votes)
    scores = compute_confidence_v2(
        all_scores, ensemble["agreement_score"], top_k=top_k, filtered=filtered
    )

    return {
        "final_confidence": scores["final"],
        "band": scores["band"],
        "generator_mode": scores["generator_mode"],
        "route": route,
        "routing_label": ensemble["majority_label"],
        "retrieval_details": scores,
        ...
        "top_result": {
            "source_url": reranked[0]["source_url"] if reranked else None,
            "heading": reranked[0]["heading"] if reranked else None,
            "content_preview": reranked[0]["content"][:200] if reranked else None,
        },
    }
